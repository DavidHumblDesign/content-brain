...
const CONFIG_VERSION = 5; // bump this to force-reset config on next load
const DEFAULT_CONFIG = {
  ...
  formats: [
    { id: "short_post", label: "Short post", inst: "FORMAT: Short post\n- STRICT LENGTH: Under 80 words. This is a hard cap.\n- Every word must earn its place. One idea only. Fragments are fine.\n- Punchier > longer. If you can say it in 40 words, don't use 80.\n- This format OVERRIDES any platform length suggestions.", active: true },
    { id: "long_post", label: "Long-form post", inst: "FORMAT: Long-form post\n- STRICT LENGTH: 800-1500 words minimum. Do NOT write less than 800 words.\n- This is a substantial piece of writing — not a quick social post.\n- Structure: Hook → Story/Evidence → Insight → CTA.\n- Use line breaks, subheadings, and formatting for readability.\n- Go deep on one topic. Include examples, specifics, and detail.\n- If it feels short, it IS short. Push past 800 words.\n- This format OVERRIDES any platform length suggestions.", active: true },
    { id: "deep_dive", label: "Deep-dive article", inst: "FORMAT: Deep-dive article\n- STRICT LENGTH: 2000-5000+ words. This is a LONG article. Do NOT write less than 2000 words.\n- This is comprehensive, reference-grade content: guidelines, research breakdowns, tutorials, and reference pieces.\n- Structure with clear H2 and H3 sections. Each section should stand alone.\n- Include comparison tables, checklists, decision frameworks, and examples in every section.\n- The reader should be able to bookmark this and return to it.\n- Cover every angle. Address edge cases. Include a table of contents.\n- If it feels long enough, it probably isn't. Keep going.\n- This format OVERRIDES any platform length suggestions.", active: true },
    ...
    { id: "research_paper", label: "Research paper", inst: "FORMAT: Research-style post\n- STRICT LENGTH: 1500-3000+ words. Do NOT write less than 1500 words.\n- Structure: Thesis → Evidence → Counter-arguments → Conclusion.\n- Every claim backed by data. Use subheadings for each major argument.\n- Include at least one data table or comparison.\n- This is a thorough, well-sourced piece — not a summary.\n- This format OVERRIDES any platform length suggestions.", active: true },
    ...
    { id: "how_to_guide", label: "How-to guide", inst: "FORMAT: How-to guide\n- STRICT LENGTH: 1500-3000+ words. Do NOT write less than 1500 words.\n- Step-by-step. Number every step.\n- Each step: what to do + why it works + what to watch out for.\n- Start with prerequisites and expected outcome.\n- End with the finished result and next steps.\n- Include troubleshooting notes and edge cases.\n- This format OVERRIDES any platform length suggestions.", active: false },
...
const DEFAULT_RULES = {
  ...
  guideline: [
    ...
    { id: 7094, text: "Aim for 2000-5000+ words. A thin guideline signals incomplete coverage. If it's under 2000 words, you're missing critical considerations.", active: true },
  ],
  glossary: [
...
function buildPrompt({ approach, profile, rules, config, selPlatform, selFormat, ideaText, commands, includeHooks, includeAntiRobot, includeSeo }) {
  ...
  if (mActive.length || (includeHooks && hActive.length) || (includeAntiRobot && arActive.length) || fmtCfg || pltCfg) {
    ...
    if (fmtCfg && pltCfg) { o += `\nIMPORTANT: The FORMAT controls the length of this piece. If the format says 2000+ words, write 2000+ words regardless of what the platform typically suggests. Format length ALWAYS wins.\n`; }
